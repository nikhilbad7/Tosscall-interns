...
if($rs1)
{
	...
	{	
		$mydatetime= $row1['mergedatetime'];

		$query2 = "select * from event where ((eventtype='$type') and (city='$city') and (init_user!='$username') and (( '$mydatetime') NOT BETWEEN beforetime and timeout) and (status=1))";
...
//$query2="select * from event where eventtype='$type' and city='$city' and init_user!='$username' and ( '" . $time_str . "' ) not BETWEEN (beforetime and timeout) and status=1";
/*echo $query2;
$rs2=mysqli_query($c,$query2);
if($rs2){
		$count =0;
